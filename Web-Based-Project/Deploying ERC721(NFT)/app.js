...
window.onload = async function () {
    if (typeof window.ethereum !== 'undefined') {
        ...
        const contractAddress = '0xb8116BA02729c880EEfA83b9f2A3608048c27aB1';
...
document.getElementById('connectButton').addEventListener('click', async () => {
    try {
        ...
        document.getElementById('status').innerHTML = `Connected to MetaMask with Address: ${currentAccount}`;
    } catch (error) {
        console.error(error);
    }
});
...
document.getElementById('mintForm').addEventListener('submit', async (event) => {
    ...
    try {
        ...
        console.error(error);
        document.getElementById('status').innerHTML = 'Error while minting NFT.';
    }
});
